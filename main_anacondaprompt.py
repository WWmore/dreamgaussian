...
class AnacondaPromt: ##Hui: change class name GUI into AnacondaPromt
    ...
    def load_input(self, file): ##Hui: initialization for both GUI + AnacondaPromt
        # load image
    ...
    # no gui mode
    def train(self, iters=500): ##Hui: only in AnacondaPromt
        if iters > 0:
            self.prepare_train()
...
if __name__ == "__main__":
    ...
    AnacondaPromt(opt).train(opt.iters)
